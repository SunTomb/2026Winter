...
%% ===========================================================
\section{继续调研：Memory + 自主学习前沿补充（2024--2026）}
%% ===========================================================

基于前文已有综述，下一阶段最值得补强的并不是继续罗列“可存储记忆的方法”，而是把
\textbf{记忆载体}、\textbf{自主学习策略}与\textbf{参数巩固机制}三条线真正打通。
近两年的前沿工作显示，LLM 记忆研究正从“把历史塞进向量库”转向“如何让 Agent
主动组织、选择、压缩、迁移并内化经验”。这一变化意味着研究重点已从静态检索
系统，转向具备\emph{生命周期管理}能力的演化式记忆系统。

\subsection{应新增纳入的高价值前沿论文}

\begin{table}[h!]
\centering
\caption{建议新增纳入的前沿论文（优先级版）}
\renewcommand{\arraystretch}{1.25}
\small
\begin{tabular}{p{3.2cm}p{1.1cm}p{4.3cm}p{5.8cm}}
\toprule
\textbf{论文} & \textbf{年份} & \textbf{核心技术} & \textbf{纳入理由} \\
\midrule
HippoRAG & 2024 & KG + Personalized PageRank 的海马体启发检索 & 不是简单 RAG，而是把“长期记忆索引”建成结构化图记忆，是从检索走向记忆组织的关键过渡。 \\
LongMemEval & 2024/2025 & 长期交互记忆基准 & 明确把长期记忆拆成信息抽取、多会话推理、时间推理、知识更新、弃答五种能力，适合作为统一评测入口。 \\
Self-Rewarding LMs & 2024 & 模型自评奖励 + Iterative DPO & 自主学习的重要一跳：奖励信号不再外包给人类或独立 reward model，而由模型自己生成。 \\
SPIN & 2024 & 自博弈式微调 & 展示了“无需新增标注数据”的弱到强自提升路线，可与记忆系统结合成离线巩固模块。 \\
MemInsight & 2025 & 自主式历史记忆增强 & 不是只做 recall，而是先增强历史记忆的语义表示再检索，强调 memory augmentation 而非 memory lookup。 \\
MIRIX & 2025 & 六类记忆 + 多 Agent 协调 + 多模态记忆 & 把记忆从单一文本缓存扩展到核心记忆/情景记忆/语义记忆/程序记忆/资源记忆/知识金库，系统性很强。 \\
MemOS & 2025 & MemCube + 异构记忆统一调度 & 把参数记忆、激活记忆、明文记忆统一成“可治理资源”，适合写成系统架构层面的新趋势。 \\
LightMem & 2025/2026 & 三阶段层级记忆 + sleep-time update & 很适合补足“效率”与“巩固时机”两项短板：把在线推理和离线巩固分离。 \\
EvolveR & 2025 & 经验生命周期 + 离线自蒸馏 + 在线强化 & 把“从经验中学”明确写成闭环生命周期，连接 ExpeL / Reflexion 与 Self-Consolidation。 \\
\bottomrule
\end{tabular}
\end{table}

\subsection{新增主线一：结构化长期记忆正在取代朴素向量库}

HippoRAG 提出一种受海马体索引理论启发的长期记忆框架，用知识图谱与 Personalized
PageRank 组织经验，单步检索即可达到或超过部分迭代式方法的效果，且成本和时延都更低。
这说明“长期记忆”并不等于“更大的 embedding store”，而更像是\textbf{可遍历、可链接、
可传播的结构化记忆图}。在这一意义上，HippoRAG 与 A-MEM 形成明显呼应：前者强调
图结构索引，后者强调 Zettelkasten 式卡片链接，两者共同指向“记忆网络化”趋势。
\hfill(\href{https://arxiv.org/abs/2405.14831}{HippoRAG, arXiv:2405.14831})

MemInsight 则向前走了一步：它不满足于存历史，而是对历史交互进行\textbf{自主增强}
（autonomous augmentation），通过更丰富的语义表示提升后续检索与生成质量。相比
“原样存储 + 相似度召回”，MemInsight 更接近“记忆重写/增广”范式。若与 PAMU 的
偏好更新思想结合，可以形成“先语义增强，再偏好校正”的双阶段更新框架。
\hfill(\href{https://arxiv.org/abs/2503.21760}{MemInsight, arXiv:2503.21760})

LightMem 进一步把长期记忆系统分成感觉记忆、短时记忆与长时记忆三层，并把长期记忆
更新放到离线的 sleep-time update 中，从而将在线回答成本与记忆巩固解耦。这一点非常
关键：它提示未来 LLM Agent 的记忆维护不应总发生在对话主路径上，而可以在后台或
批处理阶段完成高成本重组与压缩。对比你当前文稿中的 Mem0、A-MEM 与 PAMU，
LightMem 最值得补写的是它对\textbf{效率—效果折中}的显式建模。
\hfill(\href{https://arxiv.org/abs/2510.18866}{LightMem, arXiv:2510.18866})

\subsection{新增主线二：自主学习开始形成“经验闭环”}

如果说 Reflexion 仍然是“试错后写一句反思”，那么 Devil's Advocate、SPIN、
Self-Rewarding 和 EvolveR 代表的是更完整的自主学习闭环。Devil's Advocate 的价值在于
把反思前移到动作之前：Agent 不只是事后总结，而是在执行前就预测潜在失败并规划补救路径，
显著减少反复试错的次数。这种 anticipatory reflection 很适合与 Memory-R1 的管理动作
空间结合，把“ADD/UPDATE/DELETE/NOOP”之前先做失败预演。
\hfill(\href{https://arxiv.org/abs/2405.16334}{Devil's Advocate, arXiv:2405.16334})

SPIN 与 Self-Rewarding 则把“学习信号从何而来”这个问题推到了前台。SPIN 用自博弈生成
新训练样本，并通过迭代区分人类数据与模型自生成数据来完成弱到强提升；Self-Rewarding
更进一步，让模型自己扮演 judge，生成 reward，再用 Iterative DPO 继续训练。这两篇论文
虽然不直接属于长期记忆论文，但它们补齐了你原文中“自主学习”维度最薄弱的一块：\textbf{
没有外部标注时，Agent 如何持续获得学习信号并迭代改进}。
\hfill(\href{https://arxiv.org/abs/2401.01335}{SPIN, arXiv:2401.01335};
\href{https://arxiv.org/abs/2401.10020}{Self-Rewarding LMs, arXiv:2401.10020})

EvolveR 则把经验学习正式写成“离线自蒸馏 + 在线交互 + 策略强化”的闭环生命周期。
它比 ExpeL 更系统，因为 ExpeL 的重点仍是经验抽取与回忆，而 EvolveR 明确要求把交互
轨迹蒸馏成可复用的策略原则，再在后续任务中主动检索与更新。这条线和 Self-Consolidation
天然相连：EvolveR 更像\textbf{策略层的经验库闭环}，Self-Consolidation 更像\textbf{参数层的
经验内化闭环}，两者合起来就是完整的 lifelong agent。
\hfill(\href{https://arxiv.org/abs/2510.16079}{EvolveR, arXiv:2510.16079};
\href{https://arxiv.org/abs/2602.01966}{Self-Consolidation, arXiv:2602.01966})

\subsection{新增主线三：记忆正在从“模块”上升为“系统资源”}

MIRIX 与 MemOS 的价值不在于单点指标，而在于它们把记忆看成\textbf{系统级资源}。
MIRIX 将记忆拆为六类并用多 Agent 协调更新和检索，说明未来记忆系统不会只有一个
“memory store”，而会区分事实、偏好、程序、资源与安全敏感信息。MemOS 则更进一步，
试图统一参数记忆、激活记忆和明文记忆，并通过 MemCube 抽象管理记忆内容、来源、
版本与迁移。这意味着“记忆写入模型权重”与“记忆保存在外部数据库”不应再被视作
两条割裂路线，而是同一内存层级中的不同存储形态。
\hfill(\href{https://arxiv.org/abs/2507.07957}{MIRIX, arXiv:2507.07957};
\href{https://arxiv.org/abs/2507.03724}{MemOS, arXiv:2507.03724})

这一系统视角也能反向解释为什么 Self-Consolidation 值得高度重视：它不是孤立地“做了个
LoRA 微调”，而是在尝试实现记忆从明文经验向参数记忆的迁移。若将 MemOS 的异构记忆
治理框架与 Self-Consolidation 的参数蒸馏结合，可以得到一个更强的研究命题：
\textbf{记忆对象是否应当拥有类型、版本、访问权限以及迁移策略？}

\subsection{建议重写的综述主叙事}

建议将原文“记忆形成—检索—更新—整合”四段式，升级为如下六段式叙事：

\begin{enumerate}
  \item \textbf{Memory Substrate}：记忆存在哪里（向量库、图结构、分类型记忆、参数、KV/activation）；
  \item \textbf{Memory Organization}：记忆如何被链接、摘要、增广与重写（A-MEM, HippoRAG, MemInsight）；
  \item \textbf{Memory Policy}：谁决定写什么、删什么、何时检索（Memory-R1, PAMU）；
  \item \textbf{Autonomous Learning Signal}：自主学习信号从哪里来（Devil's Advocate, SPIN, Self-Rewarding, EvolveR）；
  \item \textbf{Consolidation \& Migration}：记忆如何在明文、激活、参数之间迁移（LightMem, Self-Consolidation, MemOS）；
  \item \textbf{Evaluation}：如何统一评测长期记忆与持续学习（LongMemEval, LoCoMo, WebArena 等）。
\end{enumerate}

这样的改写会让“memory + 自主学习”真正交叉起来，而不是把自主学习仅当作记忆系统的附属功能。

\subsection{建议新增的研究命题}

\paragraph{命题一：Graph-Memory + RL Policy + Parameter Consolidation}
可把 HippoRAG / A-MEM 作为结构化外部记忆底座，Memory-R1 作为操作策略层，
Self-Consolidation 作为参数内化层，形成“图记忆组织—RL 决策—参数巩固”的三层闭环。
这比单独复现某一篇论文更像真正的新工作。

\paragraph{命题二：Sleep-time Consolidation Triggering}
当前 LightMem 说明“离线巩固”有效，但\textbf{何时触发巩固}依旧开放。可以研究基于
不确定度、记忆冲突强度、调用频率与用户偏好漂移速度的触发器，使 sleep-time update
从启发式变为学习式策略。

\paragraph{命题三：Typed Memory for Lifelong Agents}
借鉴 MIRIX 和 MemOS，区分 preference memory、episodic memory、procedural memory
和 secure memory，研究不同类型记忆是否应采用不同的检索器、衰减函数与巩固方式。
这会明显优于“所有记忆统一编码成一类向量”的做法。

\subsection{建议新增的评测与实验组合}

建议不要只在 LoCoMo 上收敛结论，而应使用至少一组“对话长期记忆 + 一组工具型 Agent
持续学习”的双场景评测：
\begin{itemize}
  \item \textbf{对话长期记忆}：LoCoMo + LongMemEval；
  \item \textbf{工具型 Agent 学习}：WebArena / ALFWorld；
  \item \textbf{核心指标}：QA 准确率、长期成功率曲线、记忆库规模增长率、单位任务 token 开销、
  检索延迟、参数巩固频率、错误复发率（failure recurrence）。
\end{itemize}

其中，LongMemEval 尤其值得加入，因为它把长期记忆拆成信息抽取、多会话推理、时间推理、
知识更新和弃答五个维度，更适合作为“记忆系统是否真的会长期学习”的统一基准。
\hfill(\href{https://arxiv.org/abs/2410.10813}{LongMemEval, arXiv:2410.10813})

\subsection{一句话结论}

下一阶段最值得做的，不是再造一个“更大的记忆库”，而是构建一个\textbf{会组织记忆、会从
经验中自学、会把高价值经验巩固进参数、还能控制成本与隐私风险的记忆操作系统}。从这个
角度看，A-MEM / HippoRAG 解决“怎么组织”，Memory-R1 / EvolveR 解决“怎么学习决策”，
LightMem / Self-Consolidation / MemOS 解决“怎么巩固与迁移”，这三条线的融合最有潜力产出
真正新颖的工作。
